...
fn main() {
    ...
    runtime.spawn(async {
        ...
        block_chain = block_chain.add_block("Send 200$ To A From B".to_string()).await;
        block_chain = block_chain.add_block("Send 80$ To A From C".to_string()).await;
        block_chain = block_chain.add_block("Send 290$ To B From C".to_string()).await;
        println!("{}",block_chain);
    });
}
